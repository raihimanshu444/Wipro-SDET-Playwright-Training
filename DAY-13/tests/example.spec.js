...
//we use forEach to loop over the loginData array and run the test for each set of credentials
loginData.forEach(async data => {
  test(`Checking Login form with ${data.username}, ${data.password}`, async ({ page }) => {
    ...
    await expect(msg).toHaveText(data.expected);  //to verify if the server's response matches your expected outcome.
  });
})